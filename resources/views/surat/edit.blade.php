                ...
                <form autocomplete="off" action="{{ route('surat.update', $surat) }}" method="post">
                    ...
                    <div class="pl-lg-4">
                        <div class="form-group">
                            <label class="form-control-label">Kop Surat</label>
                            <select name="kop_surat" id="kop_surat" class="form-control">
                                <option value="">Pilih Kop Surat</option>
                                @foreach (App\KopSurat::all() as $key => $item)
                                    <option value="{{ $item->kode }}" {{ $surat->kop_surat == $item->kode ? 'selected' : '' }}>{{ $item->desa }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="pl-lg-4">
                        ...
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="custom-control-input" id="tampilkan_tembusan" name="tampilkan_tembusan" {{ $surat->tembusan ? 'checked="true"' : '' }} value="1">
                            <input type="hidden" name="tembusan" id="tembusan" value="{{ $surat->tembusan }}">
                            <label class="custom-control-label" for="tampilkan_tembusan">Tembusan</label> <a href="#" data-fancybox data-caption="Akan menampilkan tembusan"><i class="fas fa-question-circle text-blue" title="Bantuan" data-toggle="tooltip"></i></a>
                        </div>
                        <div class="custom-control custom-checkbox">
