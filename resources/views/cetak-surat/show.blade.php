...
<body>
    <div style="margin-top: 1cm; margin-bottom:0.5cm; margin-left: 1cm; margin-right: 1cm;">
        <div style="height:100px;width:100%; margin-bottom: 1rem">
            ...
            <div class="lh-20px" style="text-align: center">
                @foreach ($kop_surat as $kop)
                    @if ($surat->kop_surat == $kop->kode)
                        <span style="font-size: 14pt; font-weight: bold">PEMERINTAHAN KABUPATEN CIREBON</span><br>
                        <span style="font-size: 14pt; font-weight: bold">KECAMATAN
                            {{ Str::upper($kop->kecamatan) }}</span><br>
                        @if ($kop->kode == 9)
                            <div style="font-size: 11pt;">
                                Alamat Jl. Raya Karangsuwung No. 27 Telp.(0231) 635207 Fax. (0231) 635207
                            </div>
                            <div style="font-size: 11pt; font-style: italic">
                                Email : kecamatan.karangsembung@gmail.com KodePos : 45186
                            </div>
                        @else
                        <span style="font-size: 14pt; font-weight: bold">{{ Str::upper($kop->desa) }}</span><br>
                        <div style="font-size: 11pt; font-style: italic">
                            {{ $kop->alamat }} Telp.{{ $kop->telepon }} KodePos : {{ $kop->kodepos }}
                        </div>
                        @endif
                    @endif
                @endforeach
        ...
        <div style="margin-left: 60%; width: 40%; text-align: center; float: right;">
            <p style="margin-bottom: 34px">Karangsembung, {{ $tanggal }}<br>Yth. {{ $perihal[3] }}</p>
            <p>Di - {{ $perihal[4] }}</p>
        </div>
        @else
        ...
        try {
        if ($surat->isiSurat->where('jenis_isi','!=', 4)[$key + 1]->jenis_isi == 3 || $data_kades == true &&
        $surat->data_kades == 1) {
        ...
        <table class="table" style="margin-bottom: 1rem; margin-left: 3rem">
            <tbody>
                ...
                <tr>
                    ...
                    <td width="10px" valign="top">:</td>
                    <td style="text-align: justify" valign="top">{{ $desa->ttd ? $desa->ttd->jabatan : "Kepala Desa" }}
                    </td>
                </tr>
                <tr>
...
<br>
        <div style="margin-left: 50%; width: 50%; float: right; text-align: center">
            <p style="margin-bottom: 100px; margin-top:0px">
                ...
                {{ $desa->ttd ? $desa->ttd->jabatan : "Kepala Desa" }}
            </p>
            <p style="" class="bold underline">
        ...
        <div style="margin-right: 300px; text-align: left">
            @if ($surat->tembusan == 1)
                <div>
                    <br>
                    ...
                    <p>1. Yth. Bupati Cirebon (sebagai laporan);</p>
                    <p>2. Yth. Inspektur Kabupaten Cirebon;</p>
                    <p>3. Yth. Kepala Dinas PMD Kabupaten Cirebon.</p>
                </div>
            @endif
        </div>
    </div>
